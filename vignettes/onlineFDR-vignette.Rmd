---
title: "Managing multiple testing challenges with sequential inference using
the onlineFDR package"
...
FDR in an online manner (Javanmard and Montanari (2015), Javanmard and Montanari
(2017)).   The LOND algorithm sets the significance level $\alpha_i$  based on
the number of discoveries made so far, while LORD sets them according to the
time of the most recent disovery. This package, onlineFDR, implements these new
algorithms and provides wrappers to apply them to a historic dataset or a
growing dataset. As a comparison,  we have also provided a wrapper for
implementation of a method based on the Bonferroni approach adapated to an
...
2. The function orders the information by date order.  If there are multiple
statistical tests with the same date, within each batch these dates are
randomised by default.  

3. The function then calculates, for each statistical test, the adjusted test
level (`alphai`) at which the corresponding p-value would be declared
significant.   

4. Using the p-values provided and the alphai, the wrapper calculates an
indicator function of discoveries (`R`), where `R[i] = 1` corresponds to
hypothesis i being rejected, otherwise `R[i] = 0`.  
...
to the wrapper and the values recalculated repeating steps 1 to 5. Note that the
randomisation of the data within the previous batches will remain the same to
...
and Montanari (2017). The function provides three different versions of the
...
as presented by Javanmard and Montanari (2017).
...
and p-value ('pval'), and then call the wrapper of interest. 
...
For each statistical test, the wrapper calculates the adjusted test level
...
Javanmard and Montanari (2015, 2017) proposed two procedures for online control.
...
The LOND algorithm controls the FDR for independent $p$-values. Given an overall
...
$C \approx 0.07720838$, as proposed by Javanmard and Montanari (2017)
equation 31.


### LORD procedures {#LORD}

The LORD algorithm controls the FDR for independent $p$-values. We first fix a
...
intial 'wealth' of the procedure and $b_0 > 0$ represents the 'payout' for 
rejecting a hypothesis. We require $w_0+b_0 \leq \alpha$ for FDR control to
hold.

Javanmard and Montanari (2017) presented three different versions of LORD, which
...
and hence provably control the FDR for indepdendent p-values provided 
$w_0 + b_0 \leq \alpha$. In Javanmard and Montanari (2017), the authors also
...
where $C \approx 0.07720838$, as proposed by Javanmard and Montanari (2017)
equation 31.


#### LORDdep {#LORDdep}

Javanmard and Montanari (2017) also presented an adjusted version of LORD that
...
Note that allowing for dependent p-values results in a substantial loss in power
compared with the LORD procedures described above.
...
argument. As a default, the date format is set to recieve dates as 
...
the order of p-values in each batch are randomised.  This is to avoid the risk
of p-values being ordered post-hoc, which can lead to an inflation of the false
discovery rate.  As the dataset grows the data is reprocessed, but to ensure
consistency of output the randomisation within the previous batches remains the
same. The user has the option to turn off the randomisation step.  This approach
would be appropriate if the p-values within the batches were ordered using 
*independent* side information (so that hypotheses most likely to be rejected 
come first, which would increase the power of the procedure). 


### LOND

As a default, the `dep` argument is set to `FALSE`.  Alternatively,  this could
be set to `TRUE` and will implement the LOND algorithm to guarantee FDR control
