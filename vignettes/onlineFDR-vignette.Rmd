...
(2018) then proposed the SAFFRON procedure, which provides an adaptive method of
...
variety of trial settings. Finally, for both synchronous and asycnhronous
testing, Tian and Ramdas (2019) proposed the ADDIS algorithms which compensate
for the loss in power in the presence of conservative nulls by adaptively
...
that guarantees FDR control, as proposed by Ramdas et al. (2018). This
procedure uses a variant of SAFFRON's update rule.
...
Alpha-investing. Finally, Tian and Ramdas (2019) proposed the ADDIS procedure
...
Foster and Stine (2008) that guarantees FDR control. This procedure uses
SAFFRON's update rule with the constant \eqn{\lambda} replaced by a sequence
$\lambda_i = \alpha_i$. This is also equivalent to using the ADDIS algorithm
(see below) with $\tau = 1$ and $\lambda_i = \alpha_i$.
