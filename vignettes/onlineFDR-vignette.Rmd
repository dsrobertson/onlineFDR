...
algorithms and provides wrapper functions to apply them to a historic dataset or
a growing dataset. As a comparison,  we have also provided a wrapper function 
for implementation of a method based on the Bonferroni approach adapted to an
...
batch is randomised by default.  
...
to the wrapper function and the values recalculated repeating steps 1 to 5. Note
that the randomisation of the data within the previous batches will remain the
same to allow for reproducibility of the results.
...
and p-value ('pval'), and then call the wrapper function of interest. 
