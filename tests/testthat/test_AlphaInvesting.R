...
test_that("Errors for edge cases", {
    ...
    expect_error(AlphaInvesting(matrix(NA, nrow=2, ncol=2)),
                 "d must either be a dataframe or a vector of p-values.")
    
    expect_error(AlphaInvesting(0.1, alpha = -0.1),
                 "alpha must be between 0 and 1.")

    expect_error(AlphaInvesting(0.1, gammai = -1),
                 "All elements of gammai must be non-negative.")
    
    expect_error(AlphaInvesting(0.1, gammai=2),
    "The sum of the elements of gammai must not be greater than 1.")
    
    expect_error(AlphaInvesting(0.1, w0 = -0.01),
                 "w0 must be non-negative.")
    
    expect_error(AlphaInvesting(0.1, w0 = 1),
