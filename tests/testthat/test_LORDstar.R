...
test_that("Errors for edge cases", {
    ...
    expect_error(LORDstar(0.1, version='async', decision.times=1, alpha = -0.1),
                 "alpha must be between 0 and 1.")
    
    expect_error(LORDstar(0.1, version='async', decision.times=1, gammai = -1),
                 "All elements of gammai must be non-negative.")
    
    expect_error(LORDstar(0.1, version='async', decision.times=1, gammai=2),
                 "The sum of the elements of gammai must be <= 1.")
    
    expect_error(LORDstar(0.1, version='async', decision.times=1, w0 = -0.01),
                 "w0 must be non-negative.")
    
    expect_error(LORDstar(0.1, version='async', decision.times=1, alpha=0.05, w0=0.06),
...
test_that("Correct rejections for version async", {
    ...
    expect_identical(LORDstar(0.1, version='async', decision.times=1)$R, 0)
...
test_that("Correct rejections for version dep", {
    ...
    expect_identical(LORDstar(0.1, version='dep', lags=1)$R, 0)
