...
test2 <- Alpha_spending(test.df2, random=FALSE)$R


test_that("Errors for edge cases", {
    expect_error(Alpha_spending(matrix(NA, nrow=2, ncol=2)),
                 "d must either be a dataframe or a vector of p-values.")
    
    expect_error(Alpha_spending(0.1, alpha=2),
                 "alpha must be between 0 and 1.")
    
    expect_error(Alpha_spending(0.1, gammai=-1),
                 "All elements of gammai must be non-negative.")
    
    expect_error(Alpha_spending(0.1, gammai=2),
    "The sum of the elements of gammai must not be greater than 1.")
})

test_that("Correct rejections for sample dataframes", {
    expect_identical(Alpha_spending(test.df1)$R, 1)
